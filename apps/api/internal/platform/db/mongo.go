...
type User struct { ID, Email, Username, PasswordHash string; IsPrivate bool }
type Preference struct { Keywords, Locations, DesiredTitles []string; MinComp int; EmailOptIn, DarkMode bool }
type AppliedBy struct { UserID, Username string }
type JobPosting struct { ID, Company, Title, Location, Compensation, PostedAt, URL string; AppliedBy []AppliedBy }
type FeedDecision struct { UserID, JobID, DecisionType string; DecisionAt string }
...
func (s *Store) FindUserByEmail(email string) (User, bool) {
    ...
    for _, u := range s.Users { if strings.EqualFold(u.Email, email) { return u, true } }
    return User{}, false
}
...
func (s *Store) Feed(userID, search, sortBy string) []JobPosting {
    ...
    for _, d := range s.Decisions { if d.UserID == userID { hidden[d.JobID] = true } }
    q := strings.ToLower(strings.TrimSpace(search))
    out := make([]JobPosting, 0, len(s.Jobs))
    for _, j := range s.Jobs {
        if hidden[j.ID] { continue }
    ...
    sort.Slice(out, func(i, j int) bool {
        switch sortBy {
        case "company": return out[i].Company < out[j].Company
        case "title": return out[i].Title < out[j].Title
        case "location": return out[i].Location < out[j].Location
        case "money": return out[i].Compensation > out[j].Compensation
        default: return out[i].PostedAt > out[j].PostedAt
        }
    })
    return out
}
...
func (s *Store) Decide(userID, jobID, decision string) error {
    s.mu.Lock(); defer s.mu.Unlock()
    if _, ok := s.Jobs[jobID]; !ok { return errors.New("job not found") }
...
func (s *Store) Profile(requester, userID string) (Profile, bool) {
    s.mu.RLock(); defer s.mu.RUnlock()
    u, ok := s.Users[userID]; if !ok { return Profile{}, false }
    ...
    for _, d := range s.Decisions {
        if d.UserID == userID && d.DecisionType == "APPLIED" {
            if j, ok := s.Jobs[d.JobID]; ok { j.PostedAt = d.DecisionAt; p.AppliedJobs = append(p.AppliedJobs, j); p.TotalApplied++ }
        }
    }
    if requester != userID && p.IsPrivate { p.AppliedJobs = nil }
    return p, true
}
...
func (s *Store) UpdateMe(userID string, patch map[string]any) (Profile, bool) {
    s.mu.Lock(); defer s.mu.Unlock()
    u, ok := s.Users[userID]; if !ok { return Profile{}, false }
    if v, ok := patch["isPrivate"].(bool); ok { u.IsPrivate = v }
    s.Users[userID] = u
    if v, ok := patch["resumeFileName"].(string); ok { _ = v }
    if raw, ok := patch["preferences"].(map[string]any); ok {
        pref := s.Preferences[userID]
        if v, ok := raw["emailOptIn"].(bool); ok { pref.EmailOptIn = v }
...
func (s *Store) SearchUsers(q string) []map[string]any {
    ...
    for _, u := range s.Users {
        if n == "" || strings.Contains(strings.ToLower(u.Username), n) {
            total := 0
            for _, d := range s.Decisions { if d.UserID == u.ID && d.DecisionType == "APPLIED" { total++ } }
